...
int main()
{
    ...
    for(i = 0; i < 2; i++) { // Iterasi untuk baris matriks hasil
        for(j = 0; j < 1; j++) { // Iterasi untuk kolom matriks hasil
            for(k = 0; k < 2; k++) { // Iterasi untuk kolom matriks pertama atau baris matriks kedua
                jumlah = jumlah + matriks1[i][k] * matriks2[k][j];
            }
            // Menyimpan hasil perkalian ke matriks hasil
            hasil[i][j] = jumlah;
            jumlah = 0; // Mengatur ulang jumlah untuk elemen berikutnya
        }
    }
